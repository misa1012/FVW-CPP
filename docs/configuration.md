...
"simulation": {
    ...
    "timeScheme": "Euler",    // Wake convection scheme: "Euler" or "PredictorCorrector"
    "outputFrequency": 1,     // Write H5 output every N steps (1 = every step)
    "cutoffParam": 0.1,       // Vortex core cut-off parameter (regularization)
    "coreType": "ChordBasedCore", // Core model: "VanGarrel", "VanGarrelUnitConsistent", or "ChordBasedCore"
    "vortexModel": "Constant",    // Vortex decay: "Constant" or "GammaDecay"

    // Kutta-Joukowski iteration (optional)
    "kuttaTolerance": 1e-4,
    "kuttaMaxIterations": 200,
    "kuttaRelaxation": 0.3
}
```
...
- `PredictorCorrector`: second-order explicit predictor-corrector (Heun)
...
"simulation": {
    ...
    "timeScheme": "Euler",    // 尾迹对流格式: "Euler" 或 "PredictorCorrector"
    "outputFrequency": 1,     // 每 N 步写入一次 H5 输出 (1 = 每步都写)
    "cutoffParam": 0.1,       // 涡核截断参数 (正则化半径)
    "coreType": "ChordBasedCore", // 涡核模型: "VanGarrel" (epsilon^2), "VanGarrelUnitConsistent" (epsilon^4), 或 "ChordBasedCore" (基于弦长)
    "vortexModel": "Constant",    // 涡衰减模型: "Constant" (恒定) 或 "GammaDecay" (物理衰减)

    // Kutta-Joukowski 迭代（可选）
    "kuttaTolerance": 1e-4,
    "kuttaMaxIterations": 200,
    "kuttaRelaxation": 0.3
}

...
def main():
    ap = argparse.ArgumentParser(description="Compare line-type contributions")
    ap.add_argument("base_dir", help="Directory containing type_compare/all etc.")
    ...
    names = ["all", "shed_trailing", "trailing_only"]
    titles = ["all", "shed + trailing", "trailing only"]
    grids = [pv.read(base / "type_compare" / n / "omega_grid_timestep_3200.vti") for n in names]

    out_dir = base / "type_compare" / "compare_plots"
    ...
    panel_plot(xy, titles, out_dir / "omega_z_xy_line_type_compare.png", "xy", "omega_z", (-300, 300))
    panel_plot(yz, titles, out_dir / f"omega_z_yz_xD_{args.x_over_d:.1f}_line_type_compare.png", "yz", "omega_z", (-300, 300))
    maxmag = max(np.nanmax(arr) for _, _, arr in yzmag)
    panel_plot(yzmag, titles, out_dir / f"omega_mag_yz_xD_{args.x_over_d:.1f}_line_type_compare.png", "yz", "omega_mag", (0.0, maxmag))
